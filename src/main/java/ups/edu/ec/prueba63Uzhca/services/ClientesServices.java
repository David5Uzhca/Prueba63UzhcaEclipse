package ups.edu.ec.prueba63Uzhca.services;
...
import ups.edu.ec.prueba63Uzhca.bussines.GestionClientes;
...
public class ClientesServices {
	...
	public Response leer2(@PathParam("dni") String cedula, @PathParam("nombre") String nombre) {
		try{
			System.out.println("cedula " +  cedula + " nom=" + nombre);
			Cliente client = gClientes.getClientePorCedula(cedula);
	...
	public Response getclientes(){
		List<Cliente> cliente = gClientes.getclientes();
		if(cliente.size()>0)
			return Response.ok(cliente).build();
